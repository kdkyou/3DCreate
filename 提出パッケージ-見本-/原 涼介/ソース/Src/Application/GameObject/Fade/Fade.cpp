...
void Fade::Init()
{
	m_tex.Load("Asset/Textures/GameObject/Back/blackBack.png");
	completeFade = true;
	fadeFrame = 0;
	fadeTime = 20;
	fillDisplay = false;
...
{
	Math::Rectangle _rct = { 0, 0,(long)m_tex.GetWidth(),(long)m_tex.GetHeight() };
	Math::Color _color = { 1,1,1,m_alpha };
	KdShaderManager::Instance().m_spriteShader.DrawTex(&m_tex, 0, 0, &_rct, &_color);	
}
