...
void Character::Init()
{
	if (!m_spModel)
	{
		...
		m_pCollider->RegisterCollisionShape("Probe", m_spModel, KdCollider::TypeEvent);
	}

	m_gravity = 0;
	m_pos = {};
	Math::Matrix _rotY = Math::Matrix::CreateRotationY(DirectX::XMConvertToRadians(90));
	m_mWorld = _rotY * Math::Matrix::CreateTranslation(m_pos);
...
void Character::Update()
{
	...
	_moveVec.Normalize();
	_moveVec *= (m_moveSpd * m_moveLevel);
	m_pos += _moveVec;
